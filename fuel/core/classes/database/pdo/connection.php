...
class Database_PDO_Connection extends \Database_Connection
{
	...
	public function connect()
	{
		...
		{
			// Set the timezone
			$this->set_timezone($this->_config['timezone']);
		}
	}
	...
	{ 
		...
		// Make sure the database is connected
		$this->_connection or $this->connect();
        $this->_connection->exec("SET SESSION time_zone=" . $this->quote($timezone));        
	}
		...
		do
		{
			...
			catch (\Exception $e)
			{
				...
				if ($attempts > 0)
				{
					...
					if (strpos($e->getMessage(), 'MySQL server has gone away') !== false || $e->getCode() == 2006)// KienNH 2016/04/07 modify condition
					{
						$this->disconnect();
						$this->connect();
					}
